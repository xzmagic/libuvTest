...
uint64 hton64(uint64 h) { return isNetOrder() ? h : swap64(h); }

uint64 ntoh64(uint64 n) { return isNetOrder() ? n : swap64(n); }

uint32 hton32(uint32 h) { return isNetOrder() ? h : swap32(h); }

uint32 ntoh32(uint32 n) { return isNetOrder() ? n : swap32(n); }

uint16 hton16(uint16 h) { return isNetOrder() ? h : swap16(h); }

uint16 ntoh16(uint16 n) { return isNetOrder() ? n : swap16(n); }
...
std::string addr2Str(struct sockaddr &sockAddr) {
	uint32 ip = ntoh32(*(uint32 *)(&reinterpret_cast<sockaddr_in *>(&sockAddr)->sin_addr));
	uint16 port = ntoh16(reinterpret_cast<sockaddr_in *>(&sockAddr)->sin_port);
	char buffer[50];
	snprintf(buffer, 50, "%d.%d.%d.%d:%d", (ip >> 24) & 0xFF, (ip >> 16) & 0xFF, (ip >> 8) & 0xFF, ip & 0xFF, port);
	return std::string(buffer);
}

struct sockaddr str2Addr(std::string addr) {
	struct sockaddr sockaddr;
	struct sockaddr_in *addrV4 = reinterpret_cast<struct sockaddr_in *>(&sockaddr);
	int port = 0;
	unsigned char bytes[4] = {0};
	sscanf(addr.c_str(), "%d.%d.%d.%d:%d", (int *)&bytes[0], (int *)&bytes[1], (int *)&bytes[2], (int *)&bytes[3],
	       &port);
	addrV4->sin_port = hton16(port);
	memcpy(&addrV4->sin_addr, bytes, 4);
	return sockaddr;
}
...
struct sockaddr toAddr(std::string ip, int port) {
	...
	addrV4->sin_port = hton16(port);
	unsigned char bytes[4] = {0};
	sscanf(ip.c_str(), "%d.%d.%d.%d", (int *)&bytes[0], (int *)&bytes[1], (int *)&bytes[2], (int *)&bytes[3]);
	memcpy(&addrV4->sin_addr, bytes, 4);
	return sockaddr;
}
...
void print(std::string info, std::string color) {
	auto now = std::chrono::system_clock::now().time_since_epoch();
	time_t rawtime = std::chrono::duration_cast<std::chrono::seconds>(now).count();
	struct tm time = *localtime(&rawtime);
	char buffer[25] = {0};
	snprintf(buffer, 25, "%04d-%02d-%02d %02d:%02d:%02d:%03d", time.tm_year + 1900, time.tm_mon + 1, time.tm_mday,
	         time.tm_hour, time.tm_min, time.tm_sec, int(now.count() % 1000));
